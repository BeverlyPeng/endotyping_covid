...
class PFA(object):
    ...
    def initial(self, data, seed = False):
        ...
        print("iSEED1:", np.random.get_state()[1][0])
        if self.seed: np.random.seed(self.seed)
        print("iSEED2:", np.random.get_state()[1][0])
        print("self.s:", self.seed, round(np.random.rand(), 3))
    ...
    def train(self, iter_all=200, burn_in=100, step=1, output_folder="", outputfilename="", verbose = True):
        ...
        print("tSEED1:", np.random.get_state()[1][0])
        if self.seed: np.random.seed(self.seed)
        print("tSEED2:", np.random.get_state()[1][0])
        print("self.s:", self.seed, round(np.random.rand(), 3))
        ...
        for iter in range(iter_all):
            ...
            self.Phi = self.Update_Phi(WSZS, self.Eta, self.seed, ct)

            # ======================== Downward Pass ======================== #

            shape = np.repeat(self.r_k, self.N, axis=1)
            self.Theta = self.Update_Theta(Xt_to_t1, shape, self.seed, ct)
            ...
            if verbose: 
                if (iter > burn_in) & (np.mod(iter, step) == 0):
                    ...
                    df = pd.concat([df, pd.DataFrame({"Likelihood": [likelihood], "Likelihood mean": [likelihood_mean]})])
                else:
                    # print("Epoch {:3d} takes {:8.2f} seconds; Likelihood {:8.2f}".format(iter, end_time - start_time, likelihood))
                    df = pd.concat([df, pd.DataFrame({"Likelihood": [likelihood], "Likelihood mean": [float('nan')]})])

        sio.savemat(output_folder + outputfilename,{'Phi_mean': Phi_mean, 'Theta_mean': Theta_mean})
        df.to_csv(output_folder + outputfilename.replace(".mat", "_output.csv"), index = False)
    ...
    def Update_Phi(self, WSZS_t, Eta_t, seed, ct):

        # update Phi_t
        Phi_t_shape = WSZS_t + Eta_t
        # if seed: 
        #     if ct < 10: print("pSEED1:", np.random.get_state()[1][0])
        #     np.random.seed(seed)
        #     if ct < 10: print("pSEED2:", np.random.get_state()[1][0])
        #     if ct < 10: print("pSEED :", seed, round(np.random.rand(), 3))
        Phi_t = np.random.gamma(Phi_t_shape, 1)
        Phi_t = Phi_t / Phi_t.sum(0)

        return Phi_t

    def Update_Theta(self, Xt_to_t1_t, shape, seed, ct):

        # update Theta_t
        Theta_t_shape = Xt_to_t1_t + shape
        # if seed: 
        #     if ct < 10: print("tSEED1:", np.random.get_state()[1][0])
        #     np.random.seed(seed)
        #     if ct < 10: print("tSEED2:", np.random.get_state()[1][0])
        #     if ct < 10: print("tSEED :", seed, round(np.random.rand(), 3))
